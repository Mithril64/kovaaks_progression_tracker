    ...
    <div className="min-h-screen bg-[#edf1f5] text-ink">
      <header className="border-b border-line bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded bg-accent text-white">
              <Target size={20} aria-hidden />
            </div>
            <div className="min-w-0">
              <h1 className="truncate text-base font-semibold">Kovaak's Progression Tracker</h1>
              <p className="truncate text-xs text-slate-500">Local stats, benchmarks, and PB trends</p>
            </div>
          </div>
          <button
            className="inline-flex h-9 items-center gap-2 rounded border border-accent bg-accent px-3 text-sm font-medium text-white shadow-subtle transition hover:bg-[#1d5a66] disabled:cursor-wait disabled:opacity-70"
            type="button"
      ...
      <div className="mx-auto grid max-w-7xl gap-4 px-4 py-4 sm:px-6 lg:grid-cols-[220px_1fr]">
        <nav className="flex gap-2 overflow-x-auto lg:flex-col lg:overflow-visible">
          ...
          <div className="hidden rounded border border-line bg-white p-3 text-xs leading-5 text-slate-500 shadow-subtle lg:block">
...
function NavLink({ to, icon, label }: { to: string; icon: ReactNode; label: string }) {
  return (
    <Link
      to={to}
      className="inline-flex h-10 items-center gap-2 rounded border border-line bg-white px-3 text-sm font-medium shadow-subtle transition hover:border-accent [&.active]:border-accent [&.active]:bg-lift"
    >
      {icon}
      {label}
    </Link>
